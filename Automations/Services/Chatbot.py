...
def formatear_numero_internacional(numero, region_default="CO"):
    try:
        # Si ya viene con formato internacional
        if numero.startswith("+"):
            parsed = phonenumbers.parse(numero, None)
            return phonenumbers.format_number(parsed, phonenumbers.PhoneNumberFormat.E164)
        
        # Si empieza por 3 (celular típico en Colombia), parseamos normalmente
        if numero.startswith("3"):
            parsed = phonenumbers.parse(numero, region_default)
            return phonenumbers.format_number(parsed, phonenumbers.PhoneNumberFormat.E164)

        # Si empieza por otro número (como 1 o fijo), solo anteponer "+"
        return f"+{numero}"
...
def iniciar_flujo(inmueble_id):
    ...
    try:
        ...
        parametros = json.dumps({
            ...
            "tipo": inmueble["tipo"],
            "valor": str("${:,.0f}".format(inmueble["valor_canon"]).replace(",", ".")),
